...
class RysGitOutputLexer(RegexLexer):
	tokens = {
		'root': [
			...
			(r'#', Comment, 'comment'),
			(r'(\*?[a-zA-Z0-9]{7})( HEAD@{\d}: )(.+)', bygroups(String,
			Name.Label, Text)),	# Reflog
			(r'(\*?[a-zA-Z0-9]{7})(.+)', bygroups(String, Text)),		# Short log
			# Interactive rebase
			(r'(pick|squash|edit)( )([a-zA-Z0-9]{7})(.+)', bygroups(Keyword,
			Text, String, Text)),
			(r'...', Text), # Ellipsis
		],
		'default-log': [
